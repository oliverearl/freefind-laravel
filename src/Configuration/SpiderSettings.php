...
final readonly class SpiderSettings
{
    ...
    public function __construct(
        ...
        if ($this->userAgents === []) {
            throw new InvalidConfigurationException('At least one FreeFind spider user-agent signature must be configured.');
        }

        foreach ($this->userAgents as $userAgent) {
            if ($userAgent === '' || trim($userAgent) !== $userAgent) {
                throw new InvalidConfigurationException('FreeFind spider user-agent signatures must be non-empty strings without surrounding whitespace.');
            }
        }

        if ($this->cacheControl === '') {
            throw new InvalidConfigurationException('The FreeFind spider cache-control value must not be empty.');
        }
    }
    ...
    public static function fromConfig(array $config): self
    {
        ...
        if (! is_bool($middleware)) {
            throw new InvalidConfigurationException('The freefind-laravel.spider.middleware value must be a boolean.');
        }

        if (! is_array($userAgents) || array_is_list($userAgents) === false || ! array_all($userAgents, fn(mixed $userAgent): bool => is_string($userAgent))) {
            throw new InvalidConfigurationException('The freefind-laravel.spider.user_agents value must be a list of strings.');
        }

        if (! is_string($cacheControl)) {
            throw new InvalidConfigurationException('The freefind-laravel.spider.cache_control value must be a string.');
        }
